...
def add_opening_section(doc, client_info, project_info):
    ...
    para = doc.add_paragraph()
    # Extract city from city_state_zip (e.g., "St. Petersburg, FL 33710" -> "St. Petersburg")
    city_only = project_info.get('city_state_zip', '').split(',')[0].strip()
    
    opening_text = f'Kimley-Horn and Associates, Inc. ("Kimley-Horn" or "Consultant") submits this Letter Agreement ("Agreement") to {client_info["name"]} ("Client") for providing Civil Engineering services for the proposed {project_info["name"]} located at {project_info.get("address", "")} in {city_only}, Florida (Parcel ID: {project_info.get("parcel_id", "")}).'
